    ...
    return (
        <div className="flex justify-center space-x-1 w-full">
            ...
            {pageNumbers.map(number => (
                <Link onClick={()=> props.paginate(number)} to={`/News/${props.pagetype}/${number}`} key={number} className="px-4 py-2 text-gray-700 bg-gray-200 rounded-md hover:bg-blue-400 hover:text-white">
                    {number}
                </Link>
            ))}
